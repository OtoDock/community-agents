...
| `description` | yes | Short summary shown on the catalog card (1–3 sentences; long text gets truncated in the grid view). |
...
| `default_scope` | no | `"user"` (default) or `"agent"`. With `collaborative`, selects the agent's **visibility mode** — how chats/tasks are scoped between per-user and shared. |
| `collaborative` | no | Boolean, default `true`. `collaborative: false` + `default_scope: "user"` = Personal-only mode (each user gets their own private chats/tasks — what the personal-assistant templates use); `collaborative: false` + `default_scope: "agent"` = Shared-only (service agents). |
...
| `core_mcps` | no | `"all"` (default) or `"none"`. Every platform grants a newly installed agent its own core toolkit (memory, notifications, schedules, …) automatically — templates never list core tools in `mcps.json`. Set `"none"` to opt a single-purpose agent out entirely: it then gets only what `mcps.json` asks for. Platforms below 1.4.0 ignore this field. |

> **Manifests are engine-agnostic.** Don't pin an AI engine or model — there is
> no `default_layer` / `execution_paths` / `default_model` field. At install time
> the platform auto-enables the first engine connected on both the platform and
> the installer's account (Claude Code CLI, then Codex CLI), and the model
> defaults to Auto. The installer can change the engine afterwards in the agent's
> Config tab.
...
```json
{
  "required": [
    {"name": "schedules-mcp"},
    {"name": "github-mcp", "min_version": "1.0.0"},
...
- `min_version` (optional) — advisory minimum MCP version. Accepted by the schema and recorded, but not currently enforced at install time.
- `skills` (optional) — subset of the MCP's **manifest-declared skill IDs** (`manifest.json::skills[].id`) to enable for this agent. Omit the field (or pass an empty list) to enable all of the MCP's default skills — that's the right choice for almost every template. IDs that don't exist in the MCP's manifest are silently ignored, and an MCP that declares no `skills` in its manifest has nothing to select — don't invent entries for it.
...
`schedules-mcp`, `notifications-mcp`, `meetings-mcp`, `display-mcp`, `file-tools` (folder: `file-tools-mcp/`), `image-gen-mcp`, `image-search-mcp`, `triggers-mcp`, `mcps-mcp`, `agent-config-mcp`, `memory-mcp`.

Anything else must come from the community-mcps catalog (`camoufox`, `github-mcp`, `m365-mcp`, `notion-mcp`, etc.) — the platform will queue an admin install-request as part of the cascade.
...
3. Every MCP in `mcps.json` uses the **canonical name** (the `manifest.json::name` field, not the folder name) and exists in [OtoDock/community-mcps](https://github.com/OtoDock/community-mcps) OR is a platform-bundled custom MCP (see "Bundled custom MCPs available to every template" above). See the "MCP name vs folder name" gotcha above.
